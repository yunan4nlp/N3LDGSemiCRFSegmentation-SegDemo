...
int Segmentor::createAlphabet(const vector<Instance>& vecInsts) {
	...
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		for (int i = 0; i < curInstSize; ++i) {
			...
			vector<string> feat;
			extractLinearFeatures(feat, pInstance, i);
			int feat_num = feat.size();
			for(int featIndex = 0; featIndex < feat_num; featIndex++) { 
				m_feat_stats[feat[featIndex]]++;
			}
		}
	...
	cout << "Sparse num: " << m_feat_stats.size() << endl;
	cout << "Type num: " << m_type_stats.size() << endl;
...
void Segmentor::extractFeature(Feature& feat, const Instance* pInstance, int idx) {
	...
	extractLinearFeatures(feat.linear_features, pInstance, idx);
}

void Segmentor::extractLinearFeatures(vector<string>& features, const Instance* pInstance, int idx) {
	const vector<string>& words = pInstance->words;
	int max_word_size = words.size();
	features.push_back("[S]UNI=" + words[idx]);

	features.push_back("[S]BI=" + (idx - 1 >= 0 ? words[idx - 1] : "<start>") + "###" + words[idx]);

	features.push_back("[S]TRI=" + 
		(idx - 1 >= 0 ? words[idx - 1] : "<start>") + "###" +
		words[idx] + "###" +
		(idx + 1 < max_word_size ? words[idx + 1] : "<end>"));
...
void Segmentor::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_driver._modelparams.sparseAlpha.initial(m_feat_stats, m_options.featCutOff);
...
int main(int argc, char* argv[]) {
	...
	Segmentor segmentor();
	segmentor.m_pipe.max_sentense_size = GraphBuilder::max_sentence_length;
	if (bTrain) {
