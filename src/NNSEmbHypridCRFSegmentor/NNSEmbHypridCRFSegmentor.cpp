...
void Segmentor::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_seg_stats[unknownkey] = 1;
	m_driver._model_params._seg_alpha.initial(m_seg_stats);
	if (m_options.segFile != "")
		m_driver._model_params._segs.initial(&m_driver._model_params._seg_alpha, m_options.segFile, m_options.segEmbFineTune);
	else
		m_driver._model_params._segs.initial(&m_driver._model_params._seg_alpha, m_options.segEmbSize, m_options.segEmbFineTune);
