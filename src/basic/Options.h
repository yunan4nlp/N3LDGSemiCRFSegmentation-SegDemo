...
public:
  ...
  void setOptions(const vector<string> &vecOption) {
    ...
    for (; i < vecOption.size(); ++i) {
	  ...
	  if (pr.first == "segEmbSize")
		  segEmbSize = atoi(pr.second.c_str());
	  if (pr.first == "segEmbFineTune")
		  segEmbFineTune = (pr.second == "true") ? true : false;
  ...
  void showOptions() {
	...
	std::cout << "segEmbSize = " << segEmbSize << std::endl;
	std::cout << "segEmbFineTune = " << segEmbFineTune << std::endl;
