	...
	inline void forward(const vector<Feature>& features, bool bTrain = false){
		...
		for (int idx = 0; idx < seq_size; idx++) {
			...
			for (int dist = 0; idx + dist < seq_size && dist < max_seg_length; dist++) {
				...
				seg_inputs[offset + dist].forward(_pcg, feature.segs[dist]);

				seg_hidden1[offset + dist].forward(_pcg, &(outputseg[offset + dist]._output), &seg_inputs[offset + dist]);
			}
		}
		...
		for (int idx = 0; idx < seq_size; idx++) {
			offset = idx * max_seg_length;
			for (int dist = 0; idx + dist < seq_size && dist < max_seg_length; dist++) {
				output[offset + dist].forward(_pcg, &(seg_hidden1[offset + dist]));
